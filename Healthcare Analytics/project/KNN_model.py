...
X_train = scaling.fit_transform(X_train[columns])
X_test = scaling.transform(x_test[columns])
from sklearn.neighbors import KNeighborsClassifier
model = KNeighborsClassifier(n_neighbors=5)

# ################## to evaluate Model in case cross Validation
# cv_result = cross_validate(model, X_train, Y_train, cv=4, return_train_score=True)
# print(cv_result)
# print(cv_result['test_score'].mean())
# Y_predict = cross_val_predict(model, X_test, y_test, cv=4)
# print(accuracy_score(Y_predict, y_test))
################## to evaluate Model without cross validation
